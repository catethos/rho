...
load_library(template_key: "sfia_v8") → ask user which categories → fork_library → edit → save_library(action: "save")
...
5. **Save** — call `save_library(action: "save")` to persist.

Note: Immutable (standard) libraries cannot be edited directly — fork first.

### Path (b) — Document import

ingest_document → parse into skills → load into library table → save_library(action: "save")
...
4. **Save** — call `save_library(action: "save")` to persist.
