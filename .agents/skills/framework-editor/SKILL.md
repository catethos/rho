...
| "Save this" | **Save** | Call `save_framework(mode: "plan", year: CURRENT_YEAR)` to get save plan. **ALWAYS present the full plan to user and WAIT for approval before calling execute.** If any role has `status: "exists"`, you MUST ask: "Data Engineer already exists (2026 v1). Update it or create a new version (v2)?" Default to `action: "create"` (new version) unless user explicitly says update. NEVER auto-choose update. |
