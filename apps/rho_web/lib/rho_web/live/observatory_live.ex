defmodule RhoWeb.ObservatoryLive do
  ...
  alias RhoFrameworks.Demos.Hiring.{Candidates, Simulation}
  ...
  def mount(%{"session_id" => sid}, session, socket) do
  ...
  defp do_mount_with_session(sid, _session, socket) do
    socket =
      assign(socket,
        active_page: :observatory,
        session_id: sid,
  ...
  defp hydrate_from_event_log(socket, session_id) do
    ...
    with :ok <- Rho.Agent.Primary.validate_session_id(session_id) do
      do_hydrate_from_event_log(socket, session_id)
    else
      {:error, _} ->
        Logger.warning(
          "[Observatory] Refusing to hydrate invalid session_id: #{inspect(session_id)}"
        )
  ...
  defp do_hydrate_from_event_log(socket, session_id) do
    require Logger
