defmodule RhoWeb.SpreadsheetLive do
  ...
  def mount(params, _session, socket) do
    ...
    socket =
      ...
      |> assign(
        :agent_avatar,
        Rho.CLI.Config.load_avatar(:spreadsheet) || load_avatar("agent_avatar")
      )
  ...
  def terminate(_reason, socket) do
    if sid = socket.assigns[:session_id] do
      Rho.Stdlib.Plugins.Spreadsheet.unregister(sid)
    end
  ...
  defp do_send_message(content, socket) do
    ...
    result =
      if target_id do
        ...
        Rho.Agent.Worker.submit(pid, content)
      end
  ...
  defp ensure_session(socket, nil) do
    ...
    {:ok, _pid} =
      Rho.Agent.Primary.ensure_started(new_sid, agent_name: :spreadsheet, user_id: user_id)
  ...
  defp ensure_session(socket, sid) do
    ...
    {:ok, _pid} =
      Rho.Agent.Primary.ensure_started(sid, agent_name: :spreadsheet, user_id: user_id)
  ...
  defp subscribe_and_hydrate(socket, session_id) do
    ...
    Rho.Stdlib.Plugins.Spreadsheet.register(session_id, self())
