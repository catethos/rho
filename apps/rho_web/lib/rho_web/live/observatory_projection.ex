defmodule RhoWeb.ObservatoryProjection do
  ...
  def project(socket, "rho.task.requested", data) do
    parent_id = data[:parent_agent_id] || data["parent_agent_id"]
    child_id = data[:agent_id] || data["agent_id"]
    task_id = data[:task_id] || data["task_id"]
    role = data[:role] || data["role"]
    task = to_string(data[:task] || data["task"] || "")
    preview = if String.length(task) > 120, do: String.slice(task, 0, 120) <> "...", else: task

    socket
    |> record_edge(parent_id, child_id, task_id: task_id, status: :pending)
    |> append_entry(:agent_event, parent_id, "delegated to #{child_id} (#{role}): #{preview}",
      meta: %{event: :delegation, target: child_id, role: role, task_id: task_id}
    )
  end

  def project(socket, "rho.task.accepted", data) do
    agent_id = data[:agent_id] || data["agent_id"]
    task_id = data[:task_id] || data["task_id"]

    # Update the edge status from :pending to :accepted
    edges =
      Map.new(socket.assigns.edges, fn
        {{_from, ^agent_id} = key, edge_data} when is_map(edge_data) ->
          {key, Map.put(edge_data, :status, :accepted)}

        {{_from, ^agent_id} = key, count} ->
          {key, %{count: count, status: :accepted}}
    ...
    |> append_entry(
      :agent_event,
      agent_id,
      "accepted task#{if task_id, do: " #{task_id}", else: ""}",
      meta: %{event: :task_accepted, task_id: task_id}
    )
  end

  def project(socket, "rho.task.completed", data) do
    agent_id = data[:agent_id] || data["agent_id"]
    task_id = data[:task_id] || data["task_id"]
    result = to_string(data[:result] || "")
    is_error = String.starts_with?(result, "error:")
    status = if is_error, do: :error, else: :ok

    preview =
      if String.length(result) > 120,
        do: String.slice(result, 0, 120) <> "...",
        else: result

    label = if is_error, do: "task failed: #{preview}", else: "task completed: #{preview}"

    # Update the edge state for any edges pointing to this agent
    edges =
      Map.new(socket.assigns.edges, fn
        {{_from, ^agent_id} = key, edge_data} when is_map(edge_data) ->
          {key, Map.put(edge_data, :status, status)}

        {{_from, ^agent_id} = key, count} ->
          {key, %{count: count, status: status}}
    ...
    socket
    |> assign(:edges, edges)
    |> append_entry(:agent_event, agent_id, label,
      meta: %{event: :task_completed, status: status, task_id: task_id}
    )
  end
  ...
  defp record_edge(socket, from, to, opts \\ [])

  defp record_edge(socket, from, to, _opts) when is_nil(from) or is_nil(to), do: socket

  defp record_edge(socket, from, to, opts) do
    edges = Map.get(socket.assigns, :edges, %{})
    key = {from, to}

    edges =
      case {Map.get(edges, key), opts} do
        {nil, []} ->
          Map.put(edges, key, 1)

        {nil, _} ->
          Map.put(edges, key, Map.new(opts) |> Map.put(:count, 1))

        {existing, []} when is_integer(existing) ->
          Map.put(edges, key, existing + 1)

        {existing, _} when is_map(existing) ->
          Map.put(
            edges,
            key,
            Map.merge(existing, Map.new(opts)) |> Map.update(:count, 1, &(&1 + 1))
          )

        {existing, _} when is_integer(existing) ->
          Map.put(edges, key, Map.new(opts) |> Map.put(:count, existing + 1))
      end
