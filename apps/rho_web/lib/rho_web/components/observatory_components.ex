defmodule RhoWeb.ObservatoryComponents do
  ...
  def interaction_graph(assigns) do
    ...
    edge_data =
      ...
      |> Enum.with_index()
      |> Enum.map(fn {{{from, to}, edge_val}, idx} ->
        count = edge_count(edge_val)
        status = edge_status(edge_val)
        {fx, fy} = pos_map[from]
        {tx, ty} = pos_map[to]
    ...
    <div class="igraph-wrap" id="interaction-graph" phx-hook="InteractionGraph">
      <svg viewBox="0 0 240 220" class="igraph-svg">
        <defs>
          ...
          <marker id="arrow-pending" markerWidth="6" markerHeight="4" refX="5" refY="2" orient="auto">
            <path d="M0,0 L6,2 L0,4" fill="var(--text-muted)" opacity="0.6" />
          </marker>
          <marker id="arrow-accepted" markerWidth="6" markerHeight="4" refX="5" refY="2" orient="auto">
            <path d="M0,0 L6,2 L0,4" fill="var(--text-muted)" opacity="0.7" />
          </marker>
          <marker id="arrow-ok" markerWidth="6" markerHeight="4" refX="5" refY="2" orient="auto">
            <path d="M0,0 L6,2 L0,4" fill="#4ade80" opacity="0.8" />
          </marker>
          <marker id="arrow-error" markerWidth="6" markerHeight="4" refX="5" refY="2" orient="auto">
            <path d="M0,0 L6,2 L0,4" fill="#f87171" opacity="0.8" />
          </marker>
          <marker id="arrow-nil" markerWidth="6" markerHeight="4" refX="5" refY="2" orient="auto">
            <path d="M0,0 L6,2 L0,4" fill="var(--text-muted)" opacity="0.6" />
          </marker>
        </defs>
        ...
        <%= for e <- @edge_data do %>
          <line
            x1={e.x1} y1={e.y1}
            x2={e.x2} y2={e.y2}
            class={"igraph-edge igraph-edge-#{e.role} igraph-edge-#{e.status}"}
  ...
  defp edge_count(n) when is_integer(n), do: n
  defp edge_count(%{count: c}), do: c
  defp edge_count(_), do: 1

  defp edge_status(%{status: s}), do: s
  defp edge_status(_), do: nil
end
