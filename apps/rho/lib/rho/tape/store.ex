defmodule Rho.Tape.Store do
  ...
  @doc """
  Returns the JSONL path used to persist `tape_name`. Hierarchical tape
  names (e.g. `"sess_1/primary/agent_42"`) are safely flattened by
  URL-encoding the path separator as `%2F`, so nothing escapes the
  tapes directory or creates unintended nested directories.
  """
  @spec path_for(String.t()) :: String.t()
  def path_for(tape_name), do: tape_path(tape_name)
  ...
  def handle_call({:clear, tape_name}, _from, state) do
    ...
    # Evict any cached default view — monotonic IDs are reused after
    # clear, so a stale cache entry can appear fresh on the next read.
  ...
  # `"sess_1/primary/agent_42"` become safe single-segment filenames.
  # Round-trip is lossless: encode is idempotent because `%` is always
  # escaped first, and `decode_tape_name/1` inverts both substitutions.
  defp encode_tape_name(tape_name) when is_binary(tape_name) do
    tape_name
    |> String.replace("%", "%25")
    |> String.replace("/", "%2F")
  end

  defp decode_tape_name(encoded) when is_binary(encoded) do
    encoded
    |> String.replace("%2F", "/")
    |> String.replace("%25", "%")
  end
